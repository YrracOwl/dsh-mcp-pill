...
export function apply(ctx, config) {
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    //          plugin owns its `mcp-pill` namespace exactly as before.
    ...
    //          entry from that entry's Config schema, and a field is editable only
    //          when declared `.volatile()` (schemastery ≥ the rc.1 workspace
    //          build; the 0.1.5 line has no `volatile` at all, so the two cannot
    //          share one schema expression).
    //
    // On the newer host the resolved value therefore arrives through the entry
    // config instead of this scope. The client half is already version-portable
    // (resolveSettingsScope) and reads the form directly, so the pill keeps its
    // default-off behaviour here: `pillState.enabled` stays false until that
    // config-side read is wired. Nothing throws on either host.
    const settingsApi = sctx.settings
    if (!settingsApi || typeof settingsApi.register !== 'function') return
    try {
